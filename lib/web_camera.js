 ...
 * @param {String} url        web page url
 * @param {String} uid        保存到TFS时需要传入uid确定命名空间
 ...
 *   - data {String} pictrue path
 */
Camera.prototype.shotTFS = function (url, uid, filename, options, callback) {
  ...
  self._handleShot(url, options, function (err, s) {
    ...
    self.tfsClient.uploadFile(s, uid, filename, callback);
  });
};
...
Camera.prototype.shotStream = function (url, options, callback) {
  if (typeof options === 'function') {callback = options; options = {}; } options = options || {}; options._type = 'stream'; this._handleShot(url, options, callback); }; exports.create = function (options) {return new Camera(options); };